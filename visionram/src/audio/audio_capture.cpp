...
private:
    void ValidateConfig() const {
        if (config_.device.rfind("hw:", 0) != 0) {
            throw std::invalid_argument(
                "V8.3 ALSA capture requires an hw: PCM; default/plughw are not allowed");
        }
    ...
    void ConfigureHardware() {
        ...
        if (config_.require_exact_hw_params &&
            ...
            throw std::runtime_error(
                "ALSA adjusted requested hardware parameters; V8.3 exact mode required: "
        ...
        if (actual_channels > 65'535U || actual_period > 0xffff'ffffULL ||
            ...
            throw std::runtime_error("Negotiated ALSA parameter exceeds V8.3 type range");
        }
    ...
    void ConfigureSoftware() {
        ...
        // V8.3 video capture timestamps and media epoch use CLOCK_MONOTONIC.
...
#include "audio_capture_worker.inc"
