...
[[nodiscard]] std::runtime_error SensorSystemError(
...
SensorFrameRate V4L2SensorController::SetOrGetFrameRate(
    ...
    struct stat status{};
    if (::stat(config_.device.c_str(), &status) != 0) {
        throw SensorSystemError("stat(" + config_.device + ")");
    }
    if (!S_ISCHR(status.st_mode)) {
        throw std::runtime_error(
    ...
    if (fd < 0) {
        throw SensorSystemError("open(" + config_.device + ")");
    }
