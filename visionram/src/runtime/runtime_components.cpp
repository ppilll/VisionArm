...
#include "media/network_stream.h"
#endif
...
using visionarm::runtime::detail::g_reload_log_config;
using visionarm::runtime::detail::g_stop;
using visionarm::runtime::detail::RatePerSecond;
using visionarm::runtime::detail::ReadVmRssKb;
using visionarm::runtime::detail::RssSamples;
#if defined(VISIONARM_HAS_ALSA_AUDIO)
using visionarm::runtime::detail::AudioTimelineStats;
using visionarm::runtime::detail::DrainAudioQueue;
#endif
...
int visionarm::runtime::detail::RunRuntime(int argc, char** argv) {
    ...
    try {
        ...
        AudioTimelineStats audio_timeline_stats;
        ...
        const bool passed = detail::EvaluateRuntimePass(
            completed_requested_duration, terminated_by_signal, stats,
            broker_stats, encoder_stats, sink_stats, rga_stats, state_stats,
            control_ok, auxiliary_media_runtime_fault, audio_ok,
            audio_encode_ok, local_av_mux_ok, network_mux_ok, telemetry_ok,
        ...
        encoder.Shutdown();
        engine.Shutdown();
        return PublishRuntimeReport(
            options, passed, observed_duration_seconds, stats,
            diagnostic_report.str());
    } catch (const std::exception& error) {
        return WriteFaultRuntimeReport(
            options_parsed ? &options : nullptr, argc, argv, error.what());
    }
}
