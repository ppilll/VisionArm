...
if [ "$#" -lt 1 ] || [ "$#" -gt 2 ]; then
    echo "Usage: $0 FILE.mp4 [REPORT_PREFIX]" >&2
    exit 2
fi

INPUT=$1
PREFIX=${2:-${INPUT%.*}.validation}
MAX_AV_DURATION_DELTA_MS=${MAX_AV_DURATION_DELTA_MS:-150}
...
[ -s "$INPUT" ] || { echo "input file missing or empty: $INPUT" >&2; exit 1; }
...
atime_base=$(ffprobe -v error -select_streams a:0 \
    -show_entries stream=time_base -of default=nw=1:nk=1 "$INPUT")
vstart=$(ffprobe -v error -select_streams v:0 \
    -show_entries stream=start_time -of default=nw=1:nk=1 "$INPUT")
astart=$(ffprobe -v error -select_streams a:0 \
    -show_entries stream=start_time -of default=nw=1:nk=1 "$INPUT")
...
[ "$vcodec" = "hevc" ] || { echo "expected HEVC video, got: $vcodec" >&2; exit 1; }
[ "$acodec" = "aac" ] || { echo "expected AAC audio, got: $acodec" >&2; exit 1; }
[ "$asample_rate" = "48000" ] || { echo "expected 48000 Hz audio, got: $asample_rate" >&2; exit 1; }
[ "$achannels" = "2" ] || { echo "expected stereo audio, got channels=$achannels" >&2; exit 1; }
        ...
        BEGIN { n=0; have_pts=0; have_dts=0; bad=0; eps=0.000001 }
        {
            ...
            if (pts != "" && pts != "N/A") {
                ...
                if (have_pts && x + eps < last_pts) {
                    printf "%s PTS regression at packet %d: %.9f < %.9f\n", name,n,x,last_pts > "/dev/stderr"
                    bad=1
                }
...
awk -v d="$duration_delta_ms" -v limit="$MAX_AV_DURATION_DELTA_MS" \
    ...
    echo "A/V duration delta ${duration_delta_ms} ms exceeds ${MAX_AV_DURATION_DELTA_MS} ms" >&2
    exit 1
}
...

{
    ...
    echo "video_start_time=$vstart"
    echo "audio_start_time=$astart"
    ...
    echo "v8_3_av_file_validation=PASS"
} | tee "$SUMMARY"
