...
/*
 * Linear RAW10 3864x2192 @ 60 fps candidate.
 *
 * Keep the linear-mode semantic registers from the validated 891 Mbps
 * profile, and switch only the clock / HMAX / MIPI timing fields to the
 * 1485 Mbps working point already present in this BSP.
 *
 * 37.125 MHz XVCLK, 4 lanes, VMAX=2250, HMAX=550.
 */
...
/*
 * Linear RAW10 3864x2192 nominal 90 fps candidate.
 *
 * This is all-pixel sensor readout. VisionArm exposes this sensor profile
 * only for the 1920x1080@90 product mode and RKISP performs the downscale.
 *
 * 37.125 MHz INCK, 4 lanes, 2376 Mbps/lane, VMAX=2250, HMAX=366.
 * The physical sensor cadence is about 90.164 fps; V4L2 advertises the
 * product's nominal 90 fps and DQBUF timestamps remain the actual-FPS truth.
 */
...
static const struct imx415_mode supported_modes[] = {
	...
	{
		/* 4-lane RAW10 linear high-speed profile: 3864x2192 @ 60 fps. */
		.bus_fmt = MEDIA_BUS_FMT_SGBRG10_1X10,
		.width = 3864,
		.height = 2192,
		.max_fps = {
	...
	{
		/* 4-lane RAW10 all-pixel profile: nominal 3864x2192 @ 90 fps. */
		.bus_fmt = MEDIA_BUS_FMT_SGBRG10_1X10,
		.width = 3864,
		.height = 2192,
		.max_fps = {
		...
		.exp_def = 0x08ca - 0x08,
		/* Virtual line length documented for 2376 Mbps / 4-lane mode. */
		.hts_def = 4392,
		.vts_def = 0x08ca,
		.global_reg_list = imx415_global_10bit_3864x2192_regs,
...
imx415_find_best_fit(struct imx415 *imx415, struct v4l2_subdev_format *fmt)
{
	...
	/*
	 * Multiple timing profiles may share the same geometry/bus format.
	 * Preserve an already selected FPS profile when S_FMT is repeated.
	 */
	if (imx415->cur_mode &&
	    imx415->cur_mode->width == framefmt->width &&
	    imx415->cur_mode->height == framefmt->height &&
	    imx415->cur_mode->bus_fmt == framefmt->code)
					 ...
					 struct v4l2_subdev_frame_interval *fi)
{
	...
	dev_info(&imx415->client->dev,
		 "set frame interval: %d fps, link_freq=%lld, hts_def=%u, vts_def=%u\n",
		 fps, (long long)link_freq_items[mode->mipi_freq_idx],
